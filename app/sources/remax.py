"""RE/MAX Belgium source adapter — Tier 2."""
...
class RemaxSource(BaseSource):
    """Scraper for RE/MAX Belgium — major international franchise network."""

    name = "REMAX"
    tier = 2
    pool_filtered_in_url = True  # URL already filters by pool

    _SEARCH_URL = "https://www.remax.be/fr/acheter/maisons"
        ...
        for card in cards:
            try:
                ...
                if url and not url.startswith("http"):
                    url = f"https://www.remax.be{url}"

                title_el = card.select_one("h2, h3, .card-title, .property-name")

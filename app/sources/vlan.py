"""Vlan source adapter — Tier 3 (Belgian classifieds)."""
...
class VlanSource(BaseSource):
    """Scraper for Vlan.be — Belgian classifieds platform with real estate section."""

    name = "Vlan"
    tier = 3

    _SEARCH_URL = "https://www.vlan.be/fr/immobilier/maisons-a-vendre"
        ...
        for card in cards:
            try:
                ...
                if url and not url.startswith("http"):
                    url = f"https://www.vlan.be{url}"

                title_el = card.select_one("h2, h3, .ad-title, .classified-title")
                ...
                # Vlan ads often embed postal code in title or city
                text = card.get_text()
